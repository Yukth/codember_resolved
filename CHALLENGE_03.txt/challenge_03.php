...
$verify_passwords = '8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
function verify_passwords_fx(&$data){
    $verify_data = substr_count($data['password'], $data['must_contain_rule']);

    $data['is_valid_password'] = ($data['repeat_interval_rule'][0] <= $verify_data) && ($verify_data <= $data['repeat_interval_rule'][1]) ? true : false;

    return ($data['repeat_interval_rule'][0] <= $verify_data) && ($verify_data <= $data['repeat_interval_rule'][1]) ? true : false;
}

function main_explode_data($data){
    $explode_intermediatre_array = [];

    $explode_main = explode_methods(':',$data);
    $explode_main_verify_rules = explode_methods(" ",$explode_main[0]);
    
    $explode_intermediatre_array['raw'] = $data;
    $explode_intermediatre_array['password'] = trim($explode_main[1]);
    $explode_intermediatre_array['must_contain_rule'] = $explode_main_verify_rules[1];
    $explode_intermediatre_array['repeat_interval_rule'] = explode_methods('-',$explode_main_verify_rules[0]);
    $status = verify_passwords_fx($explode_intermediatre_array);

    return ['status' => $status, 'data' => $explode_intermediatre_array];
}

function process_passwords($data){
    $clean_to_process_data = [];
    $tmp_data = null;

    foreach ($data as $data_value) {
        $tmp_data = main_explode_data($data_value);
        if($tmp_data['status'] === false){
            $clean_to_process_data[] = $tmp_data['data'];
        }
    }

    return $clean_to_process_data;
}

function verify_passwords_fxs($data){
    $process_data = explode_methods(PHP_EOL,$data,false);
    $return_data = process_passwords($process_data);

    return $return_data;
}

var_dump( verify_passwords_fxs($verify_passwords) );